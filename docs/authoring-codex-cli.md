...
method twice with the Codex CLI: once to **create a new project from an empty
...
One Codex-specific note up front: Codex has no slash commands, so the shipped
workflows are **skills invoked by `$name` reference** — you write `$grillme` or
`$codify` in your message and Codex picks the skill up from `.agents/skills/`.
...
- The [`ralph`](https://github.com/ikigenba/ralph) harness for the unattended
  ...
  [codex-cli.md](codex-cli.md) shows.

## Part 1 — a new project from scratch

### 1. Make a repo and carry the method in

The spec-process skills travel by copying the agent trees out of this repo:

```sh
mkdir grump && cd grump && git init
cp -r ../idgen/.agents .
cp -r ../idgen/.claude .    # optional: the same skills packaged for Claude Code
```
...
1. Start `codex` at the repo root and load the context: `use $spec-shapes`.
2. Describe the change in plain English: *"grump should also take a
   `--mood MOOD` flag that picks the complaint register."*
3. `$grillme`, answer the questions until settled.
