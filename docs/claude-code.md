...
- The **external-cursor** version keeps the current step in a file on disk
  ...
  closing Claude Code and resuming later — the orchestrator just re-reads the file
  and picks up where it left off. **This is the preferred version** for a real
  build.
- The **internal-cursor** version keeps the current step in the orchestrator's own
  ...
  reason about, which makes it the simpler mental model — but the run has to
