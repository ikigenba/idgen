---
name: spec-shapes
description: The project/ spec contracts — the authoritative output shapes of product, research, design, and plan, the authority boundaries between them, and the hard invariants every writer keeps. Load at session start, before discussing, grilling, or codifying a spec.
---

# Spec Shapes

This skill is the **single source of truth for what the `project/` tree looks
like**. Everything that writes or reasons about the spec — the session
discussion, a `/load grillme` interrogation, `/codify`, and the loop-prompt
generator commands — takes the shapes below from here and restates them nowhere.
...
|---|---|---|
| `product/` | `README.md` — the *why*: problem, users, scope, promises, success criteria | `/codify` (rewritten in place) |
| `research/` | `research.md` — collected external ground truth that design references | `/codify` (rewritten in place; optional) |
| `design/` | `README.md` (spine) + `INDEX.md` (manifest) + `DNN.md` (one per Decision) | `/codify` (rewritten in place) |
| `plan/` | `README.md` (rules) + `STATUS.md` (manifest + `⬜`/`✅` markers) + `phase-NN.md` (one per phase) | `/codify` (append-only) |
| `loops/` | the generated build-loop prompts + `README.md` describing the installed loop | a `/create-*-prompts` generator command |
| `README.md` | the workspace map: this folder table and pointers — thin and static | `/codify` |

The loop prompts and `loops/README.md` are **not** spec artifacts — they are
generated from the finished spec by a generator command and describe whichever
...
and no restatement of the shapes above. `/codify` writes it and keeps it true
when the structure changes; the loop overview belongs to the generator command
