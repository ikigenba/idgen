...
convention is `project/loops/{gather,build,verify}.md`, with all workspace paths
...
- `project/loops/gather.md` — the **only** prompt that reads the big docs. Greps STATUS.md
  ...
  self-contained `project/loops/brief.md` (contract region + empty feedback region). Returns `NEXT`.
- `project/loops/build.md` — reads **only** the brief (contract *and* `verify`'s feedback
  region — open gaps first). Builds the package, writes id-tagged tests, runs the
  suite, commits. Leaves the marker untouched. `NEXT`.
- `project/loops/verify.md` — the independent gate; the **only** prompt that flips a
...
`project/loops/brief.md` is the **seam** that keeps build's context tiny:
...
Run it (from the service root): `ralph project/loops/gather.md project/loops/build.md project/loops/verify.md`
...
/product-mode ─► /research-mode ─► /design-mode ─► /plan-mode ─► /create-gather-build-verify-prompts
   product/         research/         design/*         plan/*        loops/{gather,build,verify}.md
                                                                         │
                       (from the service root)  ralph project/loops/gather.md project/loops/build.md project/loops/verify.md
