...
When this mode is entered, acknowledge it and **stop — wait for my instructions** before doing anything. Don't start interviewing until I tell you the feature or topic.

This mode is **re-usable**: the product can evolve. If `docs/product.md` already exists, read it first and treat it as the current state. As the goal changes, **edit that doc in place to align with the new goal — never append**. The doc is always a single, coherent statement of the current product, not a changelog.
...
When we're done, write `docs/product.md` in the shape below. Report the path.

## Output shape

`docs/product.md` owns **intent** — *why* this exists, *for whom*, what is in and out of scope, and what we **promise** the user — stated once, in **outcome terms**. It must NOT state mechanism, exact formats, exit codes, or test assertions; those belong to `docs/design.md`. Where the two could overlap (behavior), product states the *promise*; design states the *exact, checkable proof of that promise*. This boundary is load-bearing — it is what keeps product, design, and plan from overlapping.
