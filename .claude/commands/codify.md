---
description: Codify the discussed goal into the project/ spec — one automated pass, no interviewing
---
First load the `spec-shapes` skill — read `.claude/library/spec-shapes/SKILL.md`,
falling back to `~/.claude/library/spec-shapes/SKILL.md` — if it is not already in
...
**Codify is the "go do the work" step.** It assumes the goal is already settled —
discussed in this session, usually sharpened by a `/load grillme` interrogation —
...
Codify trades interaction for speed, never correctness for speed: every shape
and hard invariant in `spec-shapes` holds exactly — the authority boundaries,
...
phases start at 01 — create the full structure from the shapes in `spec-shapes`,
including the thin `project/README.md` workspace map. (`project/loops/` is not
yours: a `/create-*-prompts` generator command writes it, as a separate,
...
5. **Plan — APPEND the new phases.** For the newly-designed work, append
   ...
   `spec-shapes`: every design id in exactly one phase.
...
- **Do NOT write, regenerate, or edit `project/loops/`.** The loop prompts and
  `loops/README.md` belong to the `/create-*-prompts` generator commands. There
  is no step in a codify run that touches a loop file.
- Everything else is `spec-shapes`' law: scope boundary, authority boundaries,
...
Codify runs to completion automatically. It makes the reasonable calls without
