...
Read `project/product/product.md` for context. Decompose the open questions, then **fan out** — dispatch one subagent per question in parallel, each with a complete cold-start brief, each returning a distilled summary. Keep the main thread lean; never pull raw files or search dumps into it. Cover the codebase, prior art, and external sources.
