---
description: Create the single-prompt audit loop that adversarially verifies every design Verification id is genuinely tested
---
First load the `spec-shapes` skill (the `project/` artifact contracts) — read `.claude/library/spec-shapes/SKILL.md`, falling back to `~/.claude/library/spec-shapes/SKILL.md` — then proceed.

Create the **single-prompt audit loop** an unattended harness (`ralph`) re-invokes with a **fresh context** every turn to adversarially audit the project's test coverage one design Decision at a time: `project/loops/audit.md`. The audit answers a stronger question than "does a tagged test exist?" — for every minted `R-XXXX-XXXX` id it judges whether the tagged test *actually proves the behavior the design states*, escalating to mutation testing in a scratch worktree when reading alone cannot settle whether the test can fail. This command also updates **`project/loops/README.md`** with an audit-loop section (see the last section).

Assume the `project/` structure already exists in the shapes `spec-shapes` defines. This command only writes `project/loops/audit.md` and the README section; it does not audit anything, never edits the spec, and never touches the build loop's prompts.
...
**Frontmatter is entirely an operator option — never add any.** This command never writes a frontmatter block of its own, for any reason; which keys the prompt carries (`harness:`/`model:` backend routing, `max-retries:`, anything else ralph accepts) is the operator's choice alone. The command's only frontmatter duty is preservation: if a prompt being replaced carries a frontmatter block, copy it over verbatim into the regenerated prompt — regeneration must never strip or alter the operator's keys.
...
## `project/loops/README.md` — add the audit-loop section (this command updates it)
