...
---
First use the project-local `$spec-shapes` skill (the `project/` artifact contracts), then proceed.
...
Assume the `project/` structure already exists in the shapes `spec-shapes` defines. This workflow only writes `project/loops/audit.md` and the README section; it does not audit anything, never edits the spec, and never touches the build loop's prompts.
