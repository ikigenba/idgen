...
`project/plan/**`). You own the **contract region** of `project/prompts/brief.md`
...
2. **Check for an in-flight brief.** If `project/prompts/brief.md` exists, read only
...
3. **Author a fresh brief** (only when step 2 fell through). Read **only** what this
   ...
   - Write `project/prompts/brief.md` to the **schema** below, with the contract
...
`grep -oE 'R-[A-Z0-9]{4}-[A-Z0-9]{4}' project/prompts/brief.md` enumerates the
