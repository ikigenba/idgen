...
| `prompts/` | the build-loop prompts `gather.md`, `build.md`, `verify.md` (+ the phase-scoped `brief.md`) | `/create-gather-build-verify-prompts` (generated) |
...
and the phase-scoped `project/prompts/brief.md`) — never in the agent's memory
between turns.

| step | reads | writes | commits | flips marker | returns |
|---|---|---|---|---|---|
| **gather** | the big docs (STATUS, one phase, its Decisions) — or just the brief header, if in-flight | `project/prompts/brief.md` contract (fresh phase only) | no | no | `NEXT`, or `DONE` if no `⬜` |
| **build** | `project/prompts/brief.md` only (contract + verify feedback) | code + co-located tests | the increment | no | `NEXT` |
...
`project/prompts/brief.md` keeps build's context tiny — the complete and only input
...
`grep -oE 'R-[A-Z0-9]{4}-[A-Z0-9]{4}' project/prompts/brief.md | sort -u`:
