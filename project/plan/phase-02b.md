...
fail-loud panic. Also closes the loop with the round-trip property/fuzz test,
which exercises `MintAt` (Phase 02a) together with `TimeOf`. The observable end
...
- R-WH5F-QJYS — round-trip property/fuzz `TimeOf(MintAt(p, t)) == t` over `[Epoch, Epoch+cycle)` and arbitrary prefixes.
...
- R-WQWM-SPWC — `FuzzTimeOf`: arbitrary strings never panic (only `ErrInvalidID` or a valid time).
